...
int main() {
    ...
    while (1) {
        ...
        if (mainChoice == 1) {
            ...
            // Clear screen right before the ballot displays so previous attempts are hidden
            CLEAR_SCREEN();
            ...
            printf("\n[SUCCESS] Your vote has been securely recorded!\n");
            printf("Press [ENTER] to lock the terminal and log out...");
            getchar(); // Pauses until user hits enter
            
            // Clear screen immediately after voting completes
            CLEAR_SCREEN();
        }
        else if (mainChoice == 3) {
            printf("\nShutting down terminal. Goodbye!\n");
