...
// Homepage
void displayHomePage(Quiz *quiz) {
    int roleChoice;   // FIX 1: was "introleChoice" (missing space/type keyword)
...
// FIX 2: Missing opening brace `{` after function signature — was "int adminLogin()//{" 
int adminLogin() {
    char username[50];
    char password[50];
...
void updateQuestion(Quiz *quiz) {
    ...
    if (questionIndex >= 0 && questionIndex < quiz->count) {
        ...
        // FIX 3: newCorrectOption must be declared before use (C89/C90 compat),
        //         and logic corrected — getchar() return is int, cast properly.
        printf("Enter new correct option (A, B, C, or D, leave blank to keep current): ");
        int ch = getchar();   // FIX 3: read as int to correctly detect '\n'
        if (ch != '\n' && ch != EOF) {
            quiz->questions[questionIndex].correctOption = (char)ch;
